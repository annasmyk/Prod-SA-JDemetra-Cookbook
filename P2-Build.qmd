...
Using the GUI therefore requires organization into *workspace*(s). These can be created [manually](https://doc.jdemetra.org/t-gui-sa-modelling-features#how-to-create-a-workspace) or via an R script, based on the following code
...
spec0 <- get_reference_specification(jsai)
...
set_reference_specification(jsap, 1L, spec3_with_td)
```
...
The {JDCruncheR} package produces this report based on the diagnostics and parameters in the “demetra_m.csv” file generated by the rjwsacruncher::cruncher_and_param function. This file can also be generated directly via the GUI using [drop-down menus](https://doc.jdemetra.org/t-gui-output).
